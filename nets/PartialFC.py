...
class PartialFC(torch.nn.Module):
    ...
    def __init__(
        ...
        self.embedding_size = conf.emd_size
        self.sample_rate: float = conf.sample_rate
        self.fp16 = conf.mixed_precision
        ...
        if self.sample_rate < 1:
            ...
            self.weight_activated = torch.nn.Parameter(torch.normal(0, 0.01, (self.num_local, self.embedding_size)))

        # margin_loss
        if isinstance(margin_loss, Callable):
            self.margin_softmax = margin_loss(conf.loss_s, conf.loss_m)
        else:
            raise
